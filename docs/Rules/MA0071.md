...
When removing the `else` block would move a local declaration to a scope where its name is already used, the code fix removes the `else` keyword but keeps the block, so the local stays in its own scope.
