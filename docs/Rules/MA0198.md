...
The `IEquatable<T>` interface the compiler implements on records is one of their interfaces, so a record that declares an interface is ambiguous: the XML documentation of the record contains both interfaces.
