...
Only declared interfaces are considered. Inherited interfaces from those declared interfaces are ignored, as well as the `IEquatable<T>` interface the compiler implements on records when it is not declared explicitly.
