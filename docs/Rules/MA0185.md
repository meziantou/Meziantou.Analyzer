...
- Any parameter has an opaque runtime type — `object`, an interface, or an unconstrained generic type parameter — because the boxed runtime value may implement `IFormattable` and format culture-sensitively. Removing the explicit `CultureInfo.InvariantCulture` would then change the result depending on the current culture.

````c#
object value = 42.5; // the runtime type (double) is culture-sensitive

// ❌ Do NOT simplify: removing InvariantCulture changes the result based on the current culture
var a = string.Create(CultureInfo.InvariantCulture, $"Value: {value}");

// ✅ Keep the explicit culture
var b = string.Create(CultureInfo.InvariantCulture, $"Value: {value}");
````
