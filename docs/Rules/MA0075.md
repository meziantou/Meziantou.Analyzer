...
A type is considered culture-sensitive when it implements `System.IFormattable` or `System.ISpanFormattable`, exposes a provider-aware `ToString` overload, or may format as a different runtime type. The latter includes values typed as `object`, an interface, a type parameter, or a non-sealed class. Sealed types that do not support culture-aware formatting are considered culture-invariant.<br/>
