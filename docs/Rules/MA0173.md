...
The rule is not reported when the value cannot be moved into a lambda, such as when it uses a `ref`/`out`/`in` parameter, a `ref` local, a variable of a `ref struct` type, the `this` reference of a `struct`, an `await` expression, or declares a variable with an `out var` argument:
