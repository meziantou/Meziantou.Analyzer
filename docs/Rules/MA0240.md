...
The semantic functions test the type or the symbol of the node they are evaluated on, whatever the document the node belongs to. A query can then select a type by what it derives from, or a symbol by its attributes or by the assembly that declares it, including the symbols declared in another assembly:
...
The functions also work on the syntax nodes that the `syntax` function returns, so `syntax(//operation:Invocation)[has-attribute('System.ObsoleteAttribute')]` selects the calls to an obsolete method. A query that uses a semantic function needs the semantic model, like an entry using the `semantic` attributes.
