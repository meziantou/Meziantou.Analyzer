...
The rule does not report a diagnostic in a `lock` statement, as you cannot use `await` in it. The lambdas and the local functions declared in a `lock` statement are still analyzed, as they do not run while the lock is held.
...
public async Task Sample()
{
    lock (_lock)
    {
        Thread.Sleep(1); // compliant, as you cannot use await in a lock statement
        _ = Task.Run(async () =>
        {
            Thread.Sleep(1); // Non compliant
            await Task.Yield();
...
By default, the rule does not report the `Add` methods called by a collection initializer, as they are not written in the code. Set `MA0042.report_collection_initializers = true` to report them (see [Configuration](#configuration)).

The blocking calls that use a conditional access (`task?.Result`, `instance?.Write()`), that are in a property pattern (`task is { Result: 1 }`), or that are the `Add` methods of a collection initializer are reported, but the code fix is not offered for them, as they cannot be replaced by an `await` expression without rewriting the code.
...
MA0042.include_extension_methods_from_not_imported_namespaces = false

# Report the Add methods called by a collection initializer when an async overload exists. No code fix is offered for them. default: false
MA0042.report_collection_initializers = false
````
