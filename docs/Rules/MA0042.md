...
The rule does not report the `Add` methods called by a collection initializer, as they are not written in the code.

The code fix is not offered when the blocking call uses a conditional access (`task?.Result`, `instance?.Write()`) or is in a property pattern (`task is { Result: 1 }`), as it cannot be replaced by an `await` expression without changing the behavior of the code.
