...
The rule does not report the calls whose predicate cannot be combined, such as `Where((x, index) => ...)`, or `FirstOrDefault(defaultValue)` whose argument is not a predicate.
