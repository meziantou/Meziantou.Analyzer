...
The code fix calls `IsNaN` on the type used by the comparison, so `doubleValue == float.NaN` becomes `double.IsNaN(doubleValue)`. When the other operand is nullable, the code fix uses `GetValueOrDefault()`, so `nullableValue == double.NaN` becomes `double.IsNaN(nullableValue.GetValueOrDefault())`, which is `false` when the value is `null`.
