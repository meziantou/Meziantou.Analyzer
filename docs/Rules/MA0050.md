...
The `throw` statements and the `await` expressions of lambdas and local functions are ignored, as they are not executed by the method.
...
The rule is not reported for an `async` iterator when an `await` comes before the end of the argument validation, as the validation cannot be moved into a method that is not `async`.

````csharp
async IAsyncEnumerable<int> Sample(string a)
{
    await Task.Delay(1);
    if (a == null)
        throw new System.ArgumentNullException(nameof(a));
