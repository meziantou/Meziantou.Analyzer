# MA0204 - Remove unnecessary partial modifier
<!-- sources -->
Sources: [RemoveUnnecessaryPartialModifierAnalyzer.cs](https://github.com/meziantou/Meziantou.Analyzer/blob/main/src/Meziantou.Analyzer/Rules/RemoveUnnecessaryPartialModifierAnalyzer.cs), [RemoveUnnecessaryPartialModifierFixer.cs](https://github.com/meziantou/Meziantou.Analyzer/blob/main/src/Meziantou.Analyzer.CodeFixers/Rules/RemoveUnnecessaryPartialModifierFixer.cs)
<!-- sources -->

This rule reports a `partial` modifier on a type when the type has a single declaration and no partial members.

The `partial` modifier can be removed when there is no other declaration to merge with.

Classes inheriting from common WPF XAML base types (`System.Windows.Controls.UserControl`, `System.Windows.Controls.Page`, `System.Windows.Window`, and `System.Windows.Application`) are excluded.

The rule also does not report classes whose `partial` modifier can be required by CsWinRT AOT/WinRT source generation, for instance when CsWinRT support or .NET MAUI types are present in the compilation and a class implements WinRT-mapped interfaces such as `IDisposable`.

## Non-compliant code

````csharp
partial class Sample
{
}
````

## Compliant code

````csharp
class Sample
{
}

// The partial members (methods, properties, indexers, events, constructors) require a partial type
partial class SampleWithPartialMember
{
    partial void OnCreated();
}
````
