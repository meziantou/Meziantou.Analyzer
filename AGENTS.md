...
To get a type by its metadata name, use `Compilation.GetTypeByMetadataName`: the type declared in the compilation wins, then the one of the core library, then the only one of the references, ignoring the types marked with `[Microsoft.CodeAnalysis.Embedded]`. Use `GetBestTypeByMetadataName` only when the type is commonly declared as an `internal` type in several assemblies, as `GetTypeByMetadataName` returns `null` when several references declare it, and can return a type that is not accessible from the compilation. This is the case of the types generated by polyfill packages such as PolySharp or Polyfill (nullable and trimming attributes, `System.Index`, `System.Threading.Lock`, `CallerArgumentExpressionAttribute`, `DefaultInterpolatedStringHandler`, ...), of the types generated as source by other packages (`JetBrains.Annotations`, CsWin32), and of the types declared by user code (`Program`).
...
private static async Task<Document> FixAsync(Document document, SyntaxNode nodeToFix, CancellationToken cancellationToken)
{
    ...
    var mySymbol = semanticModel!.Compilation.GetTypeByMetadataName("System.SomeType");
    if (mySymbol is null)
        return document; // Fix not applied — but it was already shown to the user!
    // ...
}
...
public override async Task RegisterCodeFixesAsync(CodeFixContext context)
{
    ...
    if (semanticModel.Compilation.GetTypeByMetadataName("System.SomeType") is null)
        return;
