...
public sealed class DoNotUseBlockingCallInAsyncContextFixer : CodeFixProvider
{
    ...
    private static async Task<Document> ReplaceWithMethodName(Document document, SyntaxNode nodeToFix, string methodName, CancellationToken cancellationToken)
    {
        ...
        var newExpression = ((ExpressionSyntax)generator.AwaitExpression(newNode)).Parenthesize();
    ...
    private static async Task<Document> ReplaceTaskResultWithAwait(Document document, SyntaxNode nodeToFix, CancellationToken cancellationToken)
    {
        ...
        var newExpression = ((ExpressionSyntax)generator.AwaitExpression(expr)).Parenthesize();
        editor.ReplaceNode(nodeToFix, newExpression);
    ...
    private static async Task<Document> UseTaskDelay(Document document, SyntaxNode nodeToFix, INamedTypeSymbol taskSymbol, CancellationToken cancellationToken)
    {
        ...
        var newExpression = ((ExpressionSyntax)generator.AwaitExpression(generator.InvocationExpression(generator.MemberAccessExpression(generator.TypeExpression(taskSymbol), "Delay"), delay))).Parenthesize();
        editor.ReplaceNode(nodeToFix, newExpression);
        return editor.GetChangedDocument();
    }
}
