...
public sealed class DoNotIgnoreReturnValueAnalyzer : DiagnosticAnalyzer
{
    ...
    private sealed class AnalyzerContext(Compilation compilation)
    {
        ...
        private ImmutableHashSet<string> AssemblyLevelDoNotIgnoreMethodIds { get; } = GetAssemblyLevelDoNotIgnoreMethodIds(compilation);
        ...
        public void AnalyzeInvocation(OperationAnalysisContext context)
        {
            ...
            var documentationCommentId = targetMethod.GetDocumentationCommentId();
            if (documentationCommentId is not null && AssemblyLevelDoNotIgnoreMethodIds.Contains(documentationCommentId))
            {
        ...
        private static ImmutableHashSet<string> GetAssemblyLevelDoNotIgnoreMethodIds(Compilation compilation)
        {
            var builder = ImmutableHashSet.CreateBuilder<string>(StringComparer.Ordinal);
            foreach (var attr in compilation.Assembly.GetAttributes())
            {
                if (attr.AttributeClass?.ToDisplayString(SymbolDisplayFormat.CSharpErrorMessageFormat) != "Meziantou.Analyzer.Annotations.DoNotIgnoreAttribute")
                    continue;
